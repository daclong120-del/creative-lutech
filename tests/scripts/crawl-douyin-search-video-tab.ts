...
async function main() {
  console.log("=======================================================================");
  console.log("🎯 DOUYIN SEARCH VIDEO TAB CRAWLER: 20 REAL FINANCE VIDEOS (财经)");
  console.log("=======================================================================\n");
  ...
  const session = createSessionFromRaw(rawData, "douyin-finance-search-video");
  ...
  try {
    ...
    console.log("[1/3] Mở https://www.douyin.com/search/财经?type=video...");
    await page.goto("https://www.douyin.com/search/%E8%B4%A2%E7%BB%8F?type=video", { waitUntil: "domcontentloaded", timeout: 45000 });
    ...
    while (capturedVideos.length < 20 && scrollCount < 15) {
      ...
      scrollCount++;
      console.log(`   [Scroll ${scrollCount}/15] Đã bắt được ${capturedVideos.length}/20 URL video Tài chính...`);
    }

    if (capturedVideos.length < 20) {
      console.log("[2/3] Mở https://www.douyin.com/search/理财?type=video...");
      await page.goto("https://www.douyin.com/search/%E7%90%86%E8%B4%A2?type=video", { waitUntil: "domcontentloaded", timeout: 45000 });
      await page.waitForTimeout(6000);
      ...
      while (capturedVideos.length < 20 && scroll2 < 10) {
        ...
        await page.waitForTimeout(2500);
        scroll2++;
        console.log(`   [Search 2 Scroll ${scroll2}/10] Đã bắt được ${capturedVideos.length}/20 URL video...`);
      }
    }
  } finally {
    await context.close();
...
main().catch((err) => {
  console.error("❌ Fatal Error in Douyin Search Video Tab Crawl:", err);
  process.exit(1);
});
