...
async function main() {
  console.log("=======================================================================");
  console.log("🎯 HEADFUL DOUYIN FINANCE CRAWLER (20 REAL FINANCE VIDEOS 财经)");
  console.log("=======================================================================\n");
  ...
  const session = createSessionFromRaw(rawData, "douyin-finance-headful");
  ...
  console.log("[1/3] Mở Trình duyệt Chromium (Headless = false) tìm kiếm từ khóa 财经...");
  ...
  try {
    ...
    page.on("response", async (res) => {
      ...
      if (url.includes("/aweme/v1/web/") || url.includes("/search/") || url.includes("/detail/")) {
        try {
          ...
          const matches = text.match(/\b7\d{18}\b/g);
          if (matches) {
            for (const m of matches) awemeIds.add(m);
          }
        } catch {}
      }
    });

    console.log("   -> Mở https://www.douyin.com/search/财经?type=general...");
    ...
    while (awemeIds.size < 25 && scrollAttempts < 12) {
      await page.evaluate(() => window.scrollBy(0, 1000));
      await page.waitForTimeout(2500);

      const html = await page.content();
      const pageMatches = html.match(/\b7\d{18}\b/g) || [];
      for (const m of pageMatches) awemeIds.add(m);

      scrollAttempts++;
      console.log(`   [Scroll ${scrollAttempts}/12] Đã quét được ${awemeIds.size}/20 aweme_id Douyin tài chính thực tế...`);
    }

    if (awemeIds.size < 20) {
      console.log("   -> Thử tìm kiếm từ khóa 理财 (Quản lý tài chính)...");
      await page.goto("https://www.douyin.com/search/%E7%90%86%E8%B4%A2?type=general", { waitUntil: "domcontentloaded", timeout: 45000 });
      await page.waitForTimeout(5000);

      let scroll2 = 0;
      while (awemeIds.size < 25 && scroll2 < 10) {
        await page.evaluate(() => window.scrollBy(0, 1000));
        await page.waitForTimeout(2500);
        ...
        scroll2++;
        console.log(`   [Search 2 Scroll ${scroll2}/10] Đã quét được ${awemeIds.size}/20 aweme_id...`);
      }
    }
  } finally {
    await context.close();
  ...
  console.log(`\n✅ Tổng số aweme_id Douyin tài chính thu thập được: ${awemeIds.size} IDs.`);

  if (awemeIds.size === 0) {
    console.error("❌ Chưa thu thập được aweme_id từ Douyin Search.");
    process.exit(1);
  }
...
main().catch((err) => {
  console.error("❌ Fatal Error in Headful Douyin Finance Crawl:", err);
  process.exit(1);
});
