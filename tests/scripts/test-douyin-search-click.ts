...
async function main() {
  console.log("=======================================================================");
  console.log("🎯 INTERACTIVE PLAYWRIGHT SEARCH: 20 REAL FINANCE VIDEOS (财经)");
  console.log("=======================================================================\n");
  ...
  const session = createSessionFromRaw(rawData, "douyin-finance-click");
  ...
  try {
    ...
    page.on("response", async (res) => {
      const url = res.url();
      if (url.includes("/aweme/v1/web/") || url.includes("/search/") || url.includes("/feed/")) {
        try {
            ...
            if (Array.isArray(list)) {
              for (const item of list) {
                ...
                if (videoUrl && !capturedVideos.some(v => v.id === aweme.aweme_id)) {
                  ...
                  console.log(`   ✨ [Captured Finance Search Video ${capturedVideos.length}/20]: ID=${aweme.aweme_id} | ${aweme.desc?.substring(0, 40)}`);
                }
              }
            }
          }
    ...
    console.log("[1/3] Nhập từ khóa '财经' vào thanh tìm kiếm Douyin...");
    const searchInputSelector = 'input[data-e2e="searchbar-input"], input[placeholder*="搜索"], input.search-input';
    try {
      await page.waitForSelector(searchInputSelector, { timeout: 8000 });
      await page.fill(searchInputSelector, "财经");
      await page.keyboard.press("Enter");
      console.log("   -> Đã nhấn Enter tìm kiếm từ khóa '财经'.");
    } catch {
      console.log("   -> Chuyển trực tiếp tới URL: https://www.douyin.com/search/财经...");
      await page.goto("https://www.douyin.com/search/%E8%B4%A2%E7%BB%8F", { waitUntil: "domcontentloaded" });
    }

    await page.waitForTimeout(4000);

    // Kéo cuộn trang để kích hoạt API tìm kiếm
    let scrollCount = 0;
    while (capturedVideos.length < 20 && scrollCount < 15) {
      await page.evaluate(() => window.scrollBy(0, 1000));
      await page.waitForTimeout(2500);
      scrollCount++;
      console.log(`   [Scroll ${scrollCount}/15] Đã bắt được ${capturedVideos.length}/20 URL video CDN Tài chính...`);
    }
  } finally {
    await context.close();
  }
...
main().catch((err) => {
  console.error("❌ Fatal Error in Douyin Search Click Crawl:", err);
  process.exit(1);
});
