...
async function main() {
  console.log("=======================================================================");
  console.log("🎯 DOUYIN FINANCE REGEX & SIGNED API CRAWLER (20 REAL FINANCE VIDEOS)");
  console.log("=======================================================================\n");
  ...
  const session = createSessionFromRaw(rawData, "douyin-finance-regex");
  ...
  console.log("[1/3] Khởi chạy Playwright Chromium quét mã định dạng 19 số (aweme_id) từ Douyin Search...");
  ...
  try {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      console.log(`   [Scroll ${i + 1}/8] Đã quét được ${awemeIds.size} aweme_id tài chính thực tế...`);
      if (awemeIds.size >= 30) break;
    }

    if (awemeIds.size < 20) {
      console.log("   -> Thử từ khóa https://www.douyin.com/search/理财...");
      ...
      for (let i = 0; i < 8; i++) {
        ...
        console.log(`   [Search 2 Scroll ${i + 1}/8] Đã quét được ${awemeIds.size} aweme_id...`);
        if (awemeIds.size >= 30) break;
      }
    }
  } finally {
  ...
  if (awemeIds.size === 0) {
    console.error("❌ Chưa quét được aweme_id từ trang Douyin Search.");
    process.exit(1);
  }
...
main().catch((err) => {
  console.error("❌ Fatal Error in Douyin Finance Regex Crawl:", err);
  process.exit(1);
});
